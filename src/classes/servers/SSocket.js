...
class SSocket {
	...
	onMessage(message, remoteInfo) {
		...
		if (data.type !== "connect" && !this.isAuthenticated(data.identifier)) return this.sendPacket({type: "error", message: "Server not found or not authenticated"}, remoteInfo);
		if (!serverManager.servers.has(data.identifier)) return this.sendPacket({type: "error", message: "Server not found or not authenticated"}, remoteInfo);
		let required_fields = [];
		...
		if (!server) return this.sendPacket({type: "error", message: "Server not found or not authenticated"}, remoteInfo);
