...
$jscomp.getGlobal = function (l) {
	l = [
		"object" == typeof globalThis && globalThis, l, "object" == typeof window && window,
...
$jscomp.polyfillIsolated = function (l, z, A, q) {
	...
	null != z && (l
		...
		: z !== A && ($jscomp.propertyToPolyfillSymbol[ E ] = $jscomp.IS_SYMBOL_NATIVE
...
(function (l) {
	...
	function Bb(a) {
		if (!u.__browser) {
			...
			var c = l("<div/>").css({
				    ...
				    width: 100,
				    overflow: "scroll",
			    }).append(l("<div/>").css({width: "100%", height: 10}))).appendTo("body"),
			    d = c.children(),
			    e = d.children();
	...
	function Ga(a, b, c) {
		...
		b.bSortable && (a || c)
			? a && !c ?
				(b.sSortingClass = d.sSortableAsc, b.sSortingClassJUI = d.sSortJUIAscAllowed) : !a && c
	...
	function T(a, b, c, d) {
		...
		if (k === q) {
			return a.iDrawError != e && null === g && (da(a, 0, "Requested unknown parameter " + ("function" == typeof h.mData
				? "{function}"
				: "'" + h.mData + "'") + " for row " + b + ", column " + c, 4), a.iDrawError = e), g;
		}
		if ((k === f || null === k) && null !==
			g && d !== q) {
			k = g;
		...
		var p = function (x, w) {
			...
			if (c === q || c === g) {
				f = m[ g ], k = x.innerHTML.trim(), f && f._bAttrSrc
					...
					: n ? (f._setter || (f._setter = ha(f.mData)), f._setter(d, k)) : d[ g ] = k;
			}
			g++;
		};
		if (h) {
		...
		if (-1 !== l.inArray(!1, c)) {
			...
			if (0 !== k.length) {
				...
				e = f.sZeroRecords, 1 == a.iDraw && "ajax" == Q(a)
					? e = f.sLoadingRecords
					: f.sEmptyTable && 0 === a.fnRecordsTotal() && (e = f.sEmptyTable), c[ 0 ] = l("<tr/>", {
					"class": h
						? d[ 0 ]
						: "",
				}).append(l("<td />", {valign: "top", colSpan: oa(a), "class": a.oClasses.sRowEmpty}).html(e))[ 0 ];
			}
			...
			if ("<" == g) {
				...
				if ("'" == m || '"' == m) {
					...
					-1 != n.indexOf(".")
						...
						: "#" == n.charAt(0) ? k.id = n.substr(1, n.length - 1) : k.className = n;
					t += p;
				}
				e.append(k);
				e = l(k);
	...
	function Oa(a, b, c) {
		...
		}), g, a) : a.sAjaxSource || "string" === typeof h
	...
	function Kb(a) {
		...
		var m = l("input", b).val(e.sSearch).attr("placeholder", d.sSearchPlaceholder).on("keyup.DT search.DT input.DT paste.DT cut.DT", h
			? hb(k, h)
			: k).on("mouseup", function (n) {
			setTimeout(function () {
				k.call(m[ 0 ], n);
			}, 10);
	...
	function Yb(a, b) {
		var c = a.fnFormatNumber,
		    ...
		    f = -1 === e;
		return b.replace(/_START_/g, c.call(a, d)).replace(/_END_/g, c.call(a, a.fnDisplayEnd())).replace(/_MAX_/g, c.call(a, a.fnRecordsTotal())).replace(/_TOTAL_/g, c.call(a, h)).replace(/_PAGE_/g, c.call(a, f
			? 1
			: Math.ceil(d / e))).replace(/_PAGES_/g,
	...
	function Ra(a, b, c) {
		...
		(d = 0)) : "first" == b ? d = 0 : "previous" == b ? (d = 0 <= e ? d - e : 0, 0 > d && (d = 0)) : "next" == b
			? d + e < h && (d += e)
			: "last" == b ? d = Math.floor((h - 1) / e) * e : da(a, 0, "Unknown paging action: " + b, 5);
	...
	function Ha(a) {
		...
		if (a.scrollBarVis !== fa && a.scrollBarVis !== q) {
			...
			Math.round(w.outerWidth()) < Math.round(h)
				? (la = g.scrollHeight > g.offsetHeight || "scroll" == n.css("overflow-y")
					? h + b
					: h, L && (g.scrollHeight > g.offsetHeight || "scroll" == n.css("overflow-y")) && (C.width = K(la - b)), "" !== c && "" === d || da(a, 1, "Possible column misalignment", 6))
				: la = "100%";
			p.width = K(la);
			f.width = K(la);
	...
	function ca(a, b, c) {
		for (var d = 0, e = 0, h =
			...
			for (g = c ? c[ e ].firstChild : null; f;) {
				1 === f.nodeType && (c
					? a(f, g, d)
					: a(f, d), d++), f = f.nextSibling, g = c ? g.nextSibling : null;
			}
			e++;
		}
	}
	function Za(a) {
		...
		if (d ||
			...
			for (v = 0; v < k.length; v++) {
				w = c[ k[ v ] ], m[ v ].style.width = null !== w.sWidthOrig && "" !== w.sWidthOrig
					? K(w.sWidthOrig)
					:
	...
	function cc(a) {
		...
		for (var d = 0, e = b.length; d < e; d++) {
			...
			h.bSortable && (0 < c.length && c[ 0 ].col == d ? (k.setAttribute("aria-sort", "asc" == c[ 0 ].dir
				? "ascending"
				: "descending"), h = f[ c[ 0 ].index + 1 ] || f[ 0 ]) : h = f[ 0 ], g += "asc" === h
				? a.sSortAscending
				:
				a.sSortDescending);
	...
	function nb(a, b, c, d) {
		...
		c && a.oFeatures.bSortMulti ? (c = l.inArray(b, U(e, "0")), -1 !== c
			? (b = f(e[ c ], !0), null === b && 1 === e.length && (b = 0), null === b
				? e.splice(c, 1)
				: (e[ c ][ 1 ] = h[ b ], e[ c ]._idx = b))
			: (e.push([ b, h[ 0 ], 0 ]), e[ e.length - 1 ]._idx = 0)) : e.length && e[ 0 ][ 0 ] == b
	...
	function Sa(a) {
		...
		if (e.bSort && e.bSortClasses) {
			...
			e =
				0;
			for (h = d.length; e < h; e++) {
				f = d[ e ].src, l(U(a.aoData, "anCells", f)).addClass(c + (2 > e
					? e + 1
					: 3));
			}
		}
	...
	function bc(a, b) {
		...
		for (var h, f = u.ext.type.order[ c.sType + "-pre" ], g = 0, k = a.aoData.length; g < k; g++) {
			if (c = a.aoData[ g ], c._aSortData || (c._aSortData = []), !c._aSortData[ b ] || d) {
				h = d
					? e[ g ]
					: T(a, g, b, "sort"), c._aSortData[ b ] = f ? f(h) : h;
			}
		}
	}
	function Ca(a) {
	...
	function da(a, b, c, d) {
		...
		} else if (b = u.ext, b = b.sErrMode || b.errMode, a && F(a, null, "error", [
			a, d, c,
		]), "alert" == b) {
			alert(c);
		} else {
		...
		for (d in b) {
			if (b.hasOwnProperty(d)) {
				...
				l.isPlainObject(e)
					...
					: c && "data" !== d && "aaData" !== d && Array.isArray(e) ? a[ d ] = e.slice() : a[ d ] = e;
			}
		}
		return a;
	}
	...
	function Da(a, b) {
		...
		b <= c ? a = qa(0, b) : a <= d ? (a = qa(0, c - 2), a.push("ellipsis"), a.push(b - 1)) : (a >= b - 1 - d
			? a = qa(b - (c - 2), b)
			: (a = qa(a - d + 2, a + d - 1), a.push("ellipsis"), a.push(b - 1)), a.splice(0, 0, "ellipsis"), a.splice(0, 0, 0));
		a.DT_el = "span";
		return a;
	}
	function Xa(a) {
	...
	var u  = function (a, b) {
		    ...
		    this.each(function () {
			    ...
			    if ("table" != this.nodeName.toLowerCase()) {
				    ...
				    X(r, g, [
					    "asStripeClasses", "ajax", "fnServerData", "fnFormatNumber", "sServerMethod",
					    "aaSorting", "aaSortingFixed", "aLengthMenu", "sPaginationType", "sAjaxSource", "sAjaxDataProp",
				    ...
				    null !== g.iDeferLoading && (r.bDeferLoading = !0, f = Array.isArray(g.iDeferLoading), r._iRecordsDisplay = f
					    ? g.iDeferLoading[ 0 ]
					    : g.iDeferLoading, r._iRecordsTotal = f ? g.iDeferLoading[ 1 ] : g.iDeferLoading);
				    var G = r.oLanguage;
	...
	l.extend(B.prototype, {
		any: function () {
			...
			return e.length || d ? (a = new B(g, a
				? e.concat.apply([], e)
				: e), b = a.selector, b.rows = m.rows, b.cols = m.cols, b.opts = m.opts, a) : this;
	...
	B.registerPlural = J = function (a, b, c) {
		...
		B.register(b, function () {
			...
			return d === this ? this : d instanceof B ? d.length ? Array.isArray(d[ 0 ])
				? new B(d.context, d[ 0 ])
				: d[ 0 ] : q : d;
		});
	};
	...
	var zc      = function (a, b, c, d) {
		...
		q
			? b
			: a[ 0 ] ]) && a._details && (a._details.remove(), a._detailsShow = q, a._details = q, l(a.nTr).removeClass("dt-hasChild"), qc(c));
	}, rc       = function (a, b) {
		var c = a.context;
			...
			_: function (a, b, c, d) {
				...
				l(a.nTable).on("order.dt.DT", function (e, h, f, g) {
					a === h && (e = c.idx, b.removeClass(d.sSortAsc +
						" " + d.sSortDesc).addClass("asc" == g[ e ] ? d.sSortAsc : "desc" == g[ e ]
						? d.sSortDesc
